...
for (const dir of ['candidate', 'baseline', 'diff']) {
...
function diffPng(candidatePath, baselinePath, diffPath) {
  ...
  const mismatched = pixelmatch(candidate.data, baseline.data, diff.data, candidate.width, candidate.height, {
    threshold: 0.12,
    includeAA: false,
  });
  ...
  return {
    ...
    ratio: mismatched / (candidate.width * candidate.height),
  };
}
...
const rows = results.map((result) => {
  const diff = result.visualDiff?.comparable
    ? `${(result.visualDiff.ratio * 100).toFixed(2)}%`
    : (result.visualDiff?.reason || result.baselineStatus);
