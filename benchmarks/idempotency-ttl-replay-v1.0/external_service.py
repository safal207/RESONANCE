...
def snapshot(operation_id: str, now: int) -> dict[str, object]:
    with LOCK, connect() as conn:
        effects = [dict(row) for row in conn.execute(
            "SELECT effect_id, operation_id, idempotency_key, created_at FROM effects WHERE operation_id=? ORDER BY effect_id",
            (operation_id,),
        )]
        records = [dict(row) for row in conn.execute(
            "SELECT idempotency_key, operation_id, created_at, expires_at FROM idempotency_records WHERE operation_id=? ORDER BY idempotency_key",
            (operation_id,),
        )]
...
class Handler(BaseHTTPRequestHandler):
    server_version = "RESONANCEExternalTTL/1.0"
    ...
    def do_GET(self) -> None:
        ...
        if parsed.path == "/health":
            self.send_json(200, {
                "status": "ok",
                "service": "resonance-external-http-ttl",
                "boot_id": BOOT_ID,
    ...
    def do_POST(self) -> None:
        ...
        with LOCK, connect() as conn:
            existing = conn.execute(
                "SELECT operation_id, created_at, expires_at FROM idempotency_records WHERE idempotency_key=?",
                (key,),
            ).fetchone()
            ...
            if active:
                ...
                delivery = "applied"
                conn.execute(
                    "INSERT INTO effects(operation_id, idempotency_key, created_at) VALUES (?, ?, ?)",
                    (operation_id, key, now),
                )
                conn.execute(
                    """
                    INSERT INTO idempotency_records(idempotency_key, operation_id, created_at, expires_at)
                    VALUES (?, ?, ?, ?)
                    ...
                    """,
                    (key, operation_id, now, now + ttl),
                )

            conn.execute(
                "INSERT INTO request_log(operation_id, idempotency_key, logical_time, outcome) VALUES (?, ?, ?, ?)",
                (operation_id, key, now, delivery),
            )
            if drop_ack:
                conn.execute(
                    """
...
def main() -> int:
    ...
    print(json.dumps({"service": "resonance-external-http-ttl", "boot_id": BOOT_ID, "port": args.port}), flush=True)
    try:
