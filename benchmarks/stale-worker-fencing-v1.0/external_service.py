...
def conn():
    ...
    c.execute("CREATE TABLE IF NOT EXISTS effects(id INTEGER PRIMARY KEY AUTOINCREMENT, resource_id TEXT NOT NULL, worker TEXT NOT NULL, fence INTEGER, enforce_fence INTEGER NOT NULL)")
    c.commit()
    return c
...
def snapshot(resource_id: str):
    with conn() as c:
        ...
        effects = [dict(r) for r in c.execute("SELECT id, resource_id, worker, fence, enforce_fence FROM effects WHERE resource_id=? ORDER BY id", (resource_id,))]
    return {"resource_id": resource_id, "highest_fence": int(row[0]) if row else 0, "effect_count": len(effects), "effects": effects, "status": "conflict" if len(effects) > 1 else "committed" if effects else "absent"}
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        with conn() as c:
            ...
            c.execute("INSERT INTO effects(resource_id, worker, fence, enforce_fence) VALUES (?, ?, ?, ?)", (resource_id, worker, fence, int(enforce)))
            c.commit()
        payload = snapshot(resource_id)
        payload.update({"delivery": "applied", "worker": worker, "presented_fence": fence})
