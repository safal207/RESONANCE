...
def conn():
    ...
    c.execute(
        ...
        "id INTEGER PRIMARY KEY AUTOINCREMENT, resource_id TEXT NOT NULL, worker TEXT NOT NULL, "
        "fence INTEGER, artifact_digest TEXT NOT NULL, producer_worker TEXT NOT NULL, producer_fence INTEGER NOT NULL, "
        "adoption_id TEXT, phase TEXT NOT NULL, enforce_fence INTEGER NOT NULL)"
    )
    c.commit()
    return c
...
def snapshot(resource_id: str):
    with conn() as c:
        ...
        effects = [dict(r) for r in c.execute(
            "SELECT id, resource_id, worker, fence, artifact_digest, producer_worker, producer_fence, adoption_id, phase, enforce_fence "
            "FROM effects WHERE resource_id=? ORDER BY id", (resource_id,)
        )]
    return {
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        producer_worker = self.headers.get("X-Producer-Worker", "")
        producer_fence = self.headers.get("X-Producer-Fence")
        adoption_id = self.headers.get("X-Adoption-Id")
        ...
        if not resource_id or not worker or not artifact_digest or not producer_worker or producer_fence is None:
            return self.send_json(400, {"error": "missing_identity_or_artifact"})
        producer_fence_i = int(producer_fence)
        with conn() as c:
            row = c.execute("SELECT highest_fence FROM resource_state WHERE resource_id=?", (resource_id,)).fetchone()
            ...
            c.execute(
                "INSERT INTO effects(resource_id, worker, fence, artifact_digest, producer_worker, producer_fence, adoption_id, phase, enforce_fence) "
                "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)",
                (resource_id, worker, fence, artifact_digest, producer_worker, producer_fence_i, adoption_id, phase, int(enforce)),
            )
            c.commit()
        payload = snapshot(resource_id)
        payload.update({"delivery": "applied", "worker": worker, "presented_fence": fence, "artifact_digest": artifact_digest, "adoption_id": adoption_id, "phase": phase})
