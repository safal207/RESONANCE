...
→ partial / multi-beat DMA recovery authority
→ out-of-order / overlapping DMA fragment recovery authority
→ concurrent DMA queue ordering / durable transaction-slot authority
→ queue-epoch slot reuse / cross-epoch ABA protection
→ queue-epoch wrap / authority incarnation
```
...
**Current verified milestone — 2026-08-13:** CaPU v0.33 extends verified v0.32 from fail-closed numeric queue-epoch exhaustion to bounded numeric queue-epoch wrap protected by an explicit authority incarnation. The deterministic attack creates a historical `(incarnation=1, epoch=0)`, advances through the bounded epoch namespace, then reuses numeric epoch `0` as `(incarnation=2, epoch=0)` with the same command, execution and effect IDs. Live-runtime evidence from the older incarnation is rejected/quarantined without mutating current authority; a stale pre-wrap checkpoint cannot override the current durable post-wrap identity; recovery preserves the modeled durable incarnation/epoch; and bounded incarnation exhaustion fails closed rather than wrapping. Safety passed bounded model checking to depth 24 on a reduced 2-bit identity instance; reachability passed to depth 44 with four VCD witnesses, while v0.32 executable, canonical and bounded-safety regressions remained green. The quarantine-ledger persistence claim is scoped to live-runtime stale evidence; recovery-time external inputs carry no resolution authority. Verified CaPU head: `f9d3832d84dc2415617a782cb226af83943b5ecd`. See [`Verified Report #038`](reports/verified/038-capu-queue-epoch-wrap-incarnation/REPORT.md) and its [`machine-readable result`](reports/verified/038-capu-queue-epoch-wrap-incarnation/result.json).
...
- **#034** CaPU v0.29 partial / multi-beat DMA recovery authority — bounded model-checking PASS preserving an irreversible committed beat prefix, keeping unresolved beats evidence-gated, preventing later-beat authority across an unresolved gap, applying durable issue/negative/completion evidence per beat, and permitting transaction retirement only after all four modeled beats are committed;
- **#035** CaPU v0.30 out-of-order / overlapping DMA fragment recovery authority — bounded model-checking PASS preserving exact per-fragment recovery evidence without a committed-prefix assumption, tracking durable visible-owner provenance across overlapping byte lanes, rejecting stale/foreign ownership state and permitting retirement only after all four modeled fragments are committed;
- **#036** CaPU v0.31 concurrent DMA queue ordering recovery authority — bounded model-checking PASS allowing safe younger non-overlap progress while blocking younger overlapping issue authority until older overlaps commit, preserving ordered retirement, durable visible-owner provenance and a non-reusable historical transaction-slot identity across stale-checkpoint recovery;
- **#037** CaPU v0.32 queue-epoch slot reuse / cross-epoch ABA protection — bounded model-checking PASS allowing one modeled transaction slot to be reclaimed only in the exact successor queue epoch, rejecting/quarantining prior-epoch evidence even when numeric transaction IDs are reused, preserving the current durable slot across stale-checkpoint recovery, and failing closed when the bounded queue-epoch namespace is exhausted;
- **#038** CaPU v0.33 queue-epoch wrap / authority incarnation — bounded model-checking PASS permitting numeric queue-epoch wrap only through the exact successor authority incarnation, rejecting live-runtime older-incarnation evidence even when numeric epoch and transaction IDs repeat, preserving durable post-wrap identity across stale-checkpoint recovery, and failing closed on bounded incarnation exhaustion.
