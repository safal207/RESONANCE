...
**Current verified milestone — 2026-08-13:** CaPU v0.32 extends verified v0.31 from non-reusable transaction slots inside one modeled queue epoch to a bounded one-slot lifecycle with exact-successor queue-epoch reuse. The deterministic attack deliberately reuses the same numeric command, execution and effect IDs in the successor slot, so `queue_epoch` is the decisive authority discriminator. Late prior-epoch evidence is rejected and quarantined without mutating the current `UNKNOWN` state; a stale predecessor checkpoint cannot erase or resurrect the current slot; exact current-epoch evidence controls replay/commit; and the bounded epoch namespace fails closed instead of wrapping. Safety passed bounded model checking to depth 14 on a reduced 2-bit identity instance; reachability passed to depth 28 with eight VCD witnesses, while v0.31 deterministic, canonical and bounded-safety regressions remained green. Verified CaPU head: `a3f1336825f245166e042fb14ed0f7184789a8e8`. See [`Verified Report #037`](reports/verified/037-capu-queue-epoch-slot-reuse/REPORT.md) and its [`machine-readable result`](reports/verified/037-capu-queue-epoch-slot-reuse/result.json).
