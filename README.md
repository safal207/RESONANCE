...
**Current verified milestone — 2026-08-13:** CaPU v0.29 extends verified v0.28 from one accelerator/DMA effect to one bounded ordered four-beat DMA transaction with per-beat recovery authority. A committed prefix remains non-replayable across recovery; an issued-but-unresolved beat remains `UNKNOWN`, requires discriminating evidence and blocks later beats; exact negative evidence may reopen replay only for that exact beat after the committed prefix; retry consumes the old negative receipt and creates a fresh issue witness; exact committed evidence advances the prefix; transaction retirement requires all four beats `COMMITTED` and a durable completion-receipt bitmap of `1111`. Stale partial checkpoints cannot override fresher exact per-beat issue, negative or completion evidence. Safety passed bounded model checking to depth 18 on a reduced 2-bit transaction-identity instance; reachability passed to depth 36 with ten VCD witnesses, while v0.28 deterministic, canonical and bounded-safety regressions remained green. Verified CaPU head: `64b7f26ec58e1f1f6a1f8553b458098610ec5c96`. See [`Verified Report #034`](reports/verified/034-capu-multibeat-dma-recovery/REPORT.md) and its [`machine-readable result`](reports/verified/034-capu-multibeat-dma-recovery/result.json).
