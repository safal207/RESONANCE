...
**Current verified milestone — 2026-08-13:** CaPU v0.30 removes the ordered committed-prefix assumption of v0.29 and verifies a bounded four-fragment DMA transaction whose fragments may complete out of order and overlap the same four modeled byte lanes. Exact durable issue/negative/completion evidence remains per fragment, while a durable visible-owner map records which committed fragment most recently owns each modeled lane. Recovery preserves irrevocable committed history even when newer overlapping fragments replace visible ownership; `UNKNOWN` fragments remain evidence-gated; foreign evidence is rejected; stale checkpoints cannot override fresher exact receipts or visible-owner provenance; retirement requires all four fragments `COMMITTED` with completion bitmap `1111`. Safety passed bounded model checking to depth 14 on a reduced 2-bit transaction-identity instance; reachability passed to depth 26 with ten VCD witnesses, while v0.29 deterministic, canonical and bounded-safety regressions remained green. Verified CaPU head: `f2a70b31393e52f0fbb4f43dd14739c96a8a8bd0`. See [`Verified Report #035`](reports/verified/035-capu-overlapping-dma-fragment-recovery/REPORT.md) and its [`machine-readable result`](reports/verified/035-capu-overlapping-dma-fragment-recovery/result.json).
