...
**Current verified milestone — 2026-08-13:** CaPU v0.31 extends verified v0.30 from one overlapping-fragment DMA transaction to two concurrent transactions under one modeled queue epoch. A younger non-overlapping fragment may progress while the older transaction remains unresolved, but a younger overlapping fragment cannot acquire issue authority until the older overlapping effects are committed; retirement remains ordered. Formal verification exposed transaction-slot existence/identity as an authority object distinct from fragment evidence: a stale checkpoint predating TX1 must not erase the younger slot while durable TX1 fragment/owner evidence survives, because that would permit slot identity resurrection. The bounded model therefore preserves a non-reusable durable transaction-slot identity within the queue epoch and reconstructs it across stale-checkpoint recovery. Safety passed bounded model checking to depth 12 on a reduced 2-bit identity instance; reachability passed to depth 24 with eleven VCD witnesses, while v0.30 deterministic, canonical and bounded-safety regressions remained green. The final exact-head change was formal-harness-only: a ghost ledger derived from accepted submissions replaced unsupported hierarchical observations of DUT-local durable registers; RTL and queue/recovery semantics were unchanged. Verified CaPU head: `bd3786a8a63912d34686d0ec87363caf85d60efc`. See [`Verified Report #036`](reports/verified/036-capu-concurrent-dma-queue-recovery/REPORT.md) and its [`machine-readable result`](reports/verified/036-capu-concurrent-dma-queue-recovery/result.json).
